 ...
 * CREATE FUNCTIONS
 */
IF EXISTS (SELECT * FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'FNUB_BOOK_RENTED') AND type IN ( N'FN', N'IF', N'TF', N'FS', N'FT' ))
  DROP FUNCTION FNUB_BOOK_RENTED
GO 

CREATE FUNCTION FNUB_BOOK_RENTED (
	@UserId		DECIMAL(11, 0)
...
BEGIN
	...
	FROM		TBUB_BOOK_RENTAL		RENT
	INNER JOIN	TBUB_BOOK_RENTAL_DETAIL	RDET
	ON			RDET.RENTAL_ID	=	RENT.RENTAL_ID
	WHERE		RDET.BOOK_ISBN	=	@BookIsbn
	AND			RENT.USER_ID	=	@UserId
